...
function handleSceneBtn(e, id) {
  ...
  SceneManager.initScene(newScene);
}
```
...
function handleSceneBtn(e, id) {
  ...
  if (AppState.scene.instance) {
    ...
    if (typeof SceneManager !== 'undefined') {
      SceneManager.unbindSceneEventListeners(AppState.scene.instance);
    }
...
MapView.init(container1); // First registration
MapView.init(container2); // Second registration - now fires twice!
```
...
var MapView = {
  ...
  registerWheelEvent: function() {
    ...
    if (this.wheelHandler) {
      console.log("MapView: 滾輪事件已存在，跳過重複註冊");
      return;
    }
...
MapView.init(container1); // Registers
MapView.init(container2); // Skips (already registered)
```
...
handlePreviousBtn: function(scene) {
  ...
  // ✅ Clean up MapView wheel event
  if (typeof MapView !== 'undefined') {
    MapView.removeWheelEvent();
  }
...
initScene: function(scene) {
  ...
  // scene.detailWindow animates in background (hidden)
  // scene.infoWindow animates (not visible)
  // scene.question.resultIcon animates (before quiz starts)

  this.bindSceneEventListeners(scene);
  console.log("SceneManager 場景元件初始化完成");
}
```
...
pauseListedMovieClips: function(scene) {
  ...
  if (mapContent) {
    ...
    this.pauseTargets.mapChildren.forEach(function(item) {
      for (var i = 1; i <= item.max; i++) {
        if (pauseTarget(mapContent[item.prefix + i])) {
          pausedCount++;
        }
      }
    });
