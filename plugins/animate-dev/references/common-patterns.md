...
function initScene(scene) {
  scene.mapContainer.addEventListener("mousedown", startDrag);
  scene.mapContainer.addEventListener("pressmove", handleDrag);
  scene.mapContainer.addEventListener("pressup", stopDrag);
...
function createScene(id) {
  ...
  // scene.detailWindow animating in background
  // scene.question animating when not visible
  // scene.infoWindow animating when hidden
...
function initScene(scene) {
  ...
  if (scene.detailWindow) {
    scene.detailWindow.stop();
  }

  // But many are forgotten:
  // - scene.question (still animating)
  // - scene.infoWindow (still animating)
  // - scene.mapContainer.mapContent (still animating)
  // - All item_ MovieClips inside mapContent (still animating)
}
```
